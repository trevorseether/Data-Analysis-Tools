...
DECLARE @fechacorte AS VARCHAR(8) = '20240930';
...
SELECT 
	...
	CASE
		...
		--END AS 'FACTOR MONTO DESEMBOLSADO (FMD)',
		 ...
		 CASE 
			...
			ELSE isnull(DEL_MES.MONTO_DESEMBOLSADO / metas.METAS_comercial,0) 
		 END AS 'FACTOR MONTO DESEMBOLSADO (FMD)',
	...
	CASE
		...
		--ACTUAL.SALDO_TOTAL,
		--((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) as 'deterioro mensual',

	CASE
		WHEN ACTUAL.SALDO_TOTAL = 0 THEN 0
		WHEN ((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) > 0.02					THEN 0.5
		WHEN ((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) between 0.01 AND 0.020    THEN 0.6
		WHEN ((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) between 0    AND 0.003	THEN 1

		WHEN ACTUAL.SALDO_TOTAL < 500000 THEN
			CASE WHEN (((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) between 0.006 AND 0.01)  THEN 0.7
			     WHEN (((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) between 0.003 AND 0.006)  THEN 0.8
			     WHEN (((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) < 0                  )  THEN 1.1
				 END
		WHEN ACTUAL.SALDO_TOTAL >= 500000 THEN
			CASE WHEN (((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) between 0.006 AND 0.01)  THEN 0.8
			     WHEN (((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) between 0.003 AND 0.006)  THEN 0.9
			     WHEN (((ISNULL(ACTUAL.VENCIDO_ACTUAL,0) - ISNULL(ANTERIOR.VENCIDO_ANTERIOR,0))/ACTUAL.SALDO_TOTAL) < 0                  )  THEN 1.15
				 END
	...
	CASE 
		...
		WHEN DEL_MES.MONTO_DESEMBOLSADO / metas.METAS_comercial < 0.8 THEN 'SIN RV'
	...
	CASE
		...
		WHEN (DATEDIFF(MONTH, AF.FECHA_INICIO, @fechacorte) BETWEEN  4 AND 6) THEN
			CASE WHEN (ult_12_meses.MORA_12 BETWEEN 0     AND 0.005)   THEN '1.1'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.005 AND 0.010)   THEN '1'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.010 AND 0.015)   THEN '0.9'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.015 AND 0.020)   THEN '0.7'
				 WHEN (ult_12_meses.MORA_12 > 0.02)                    THEN '0'
					END
		WHEN (DATEDIFF(MONTH, AF.FECHA_INICIO, @fechacorte) BETWEEN  7 AND 10) THEN
			CASE WHEN (ult_12_meses.MORA_12 BETWEEN 0     AND 0.015)   THEN '1.1'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.015 AND 0.020)   THEN '1'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.020 AND 0.025)   THEN '0.9'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.025 AND 0.030)   THEN '0.7'
				 WHEN (ult_12_meses.MORA_12 > 0.03)                    THEN '0'
					END
		WHEN (DATEDIFF(MONTH, AF.FECHA_INICIO, @fechacorte) > 11 ) THEN
			CASE WHEN (ult_12_meses.MORA_12 BETWEEN 0     AND 0.030)   THEN '1.1'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.030 AND 0.035)   THEN '1'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.035 AND 0.040)   THEN '0.9'
			     WHEN (ult_12_meses.MORA_12 BETWEEN 0.040 AND 0.050)   THEN '0.7'
				 WHEN (ult_12_meses.MORA_12 > 0.050)                    THEN '0'
					END
		...
		--WHEN (DATEDIFF(MONTH, AF.FECHA_INICIO, @fechacorte) >= 11)             AND (ult_12_meses.MORA_12 BETWEEN 0.029  AND 0.035) THEN '1'
	...
	CASE
		...
		WHEN DEL_MES.MONTO_DESEMBOLSADO / metas.METAS_comercial < 0.8 AND metas.METAS_comercial > 0 THEN 0

		WHEN ult_12_meses.MORA_12 > 0.05                                                              THEN 0
		WHEN (DATEDIFF(MONTH, AF.FECHA_INICIO, @fechacorte) <= 3) AND ult_12_meses.MORA_12 > 0        THEN 0

		WHEN (DEL_MES.TPPM  BETWEEN 0.28 AND 0.30) THEN
			CASE WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 5  AND 6)  THEN 200
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 7  AND 10) THEN 600
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 11 AND 13) THEN 800
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 14 AND 16) THEN 900
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 17 AND 19) THEN 1200
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) > 20)              THEN 1500
				 END
		WHEN (DEL_MES.TPPM  BETWEEN 0.3 AND 0.34) THEN
			CASE WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 5  AND 6)  THEN 300
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 7  AND 10) THEN 700
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 11 AND 13) THEN 1000
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 14 AND 16) THEN 1100
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 17 AND 19) THEN 1400
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) > 20)              THEN 1700
				 END
		WHEN (DEL_MES.TPPM  BETWEEN 0.34 AND 0.39) THEN
			CASE WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 5  AND 6)  THEN 400
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 7  AND 10) THEN 800
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 11 AND 13) THEN 1100
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 14 AND 16) THEN 1300
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 17 AND 19) THEN 1600
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) > 20)              THEN 1900
				 END
		WHEN (DEL_MES.TPPM  BETWEEN 0.39 AND 0.43) THEN
			CASE WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 5  AND 6)  THEN 500
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 7  AND 10) THEN 1000
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 11 AND 13) THEN 1300
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 14 AND 16) THEN 1500
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 17 AND 19) THEN 1800
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) > 20)              THEN 2100
				 END
		WHEN (DEL_MES.TPPM  BETWEEN 0.43 AND 0.49) THEN
			CASE WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 5  AND 6)  THEN 600
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 7  AND 10) THEN 1100
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 11 AND 13) THEN 1500
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 14 AND 16) THEN 1700
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 17 AND 19) THEN 2000
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) > 20)              THEN 2300
				 END
		WHEN (DEL_MES.TPPM  > 0.49) THEN
			CASE WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 5  AND 6)  THEN 700
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 7  AND 10) THEN 1300
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 11 AND 13) THEN 1700
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 14 AND 16) THEN 1900
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) BETWEEN 17 AND 19) THEN 2200
				 WHEN (ISNULL(DEL_MES.NÚMERO_DESEMBOLSOS,0) > 20)              THEN 2500
				 END
		...
		--	ELSE 0
			...
			fp8.fp8 AS 'TP8',
			CASE
				WHEN ACTUAL.SALDO_CARTERA < 500000 AND FP8.FP8 > 0.95   THEN 1.10
				WHEN ACTUAL.SALDO_CARTERA >= 500000 AND FP8.FP8 > 0.95  THEN 1.15
				WHEN FP8.FP8 BETWEEN 0.90 AND 0.95                      THEN 1
				WHEN FP8.FP8 BETWEEN 0.85 AND 0.90                      THEN 0.9
				WHEN FP8.FP8 BETWEEN 0.80 AND 0.85                      THEN 0.8
				WHEN FP8.FP8 <0.80                                      THEN 0.7
				ELSE 1
			END AS 'PONDERADOR TP8'
FROM 
	...
	LEFT JOIN FUNCIONARIOS..FUNCIONARIOS_ACTIVIDAD AS AF -- aquí hay que insertar los nuevos
	...
	------------------------------------------------------------------------------------------
	LEFT JOIN FUNCIONARIOS.[dbo].[METAS_20240930] AS METAS ON A.originador = METAS.FUNCIONARIO --crear uno nuevo cada mes
	LEFT JOIN FUNCIONARIOS.[dbo].[Fp8_20240930]   AS FP8   ON A.originador = FP8.[nombres para merge] --crear uno nuevo cada mes
	------------------------------------------------------------------------------------------
