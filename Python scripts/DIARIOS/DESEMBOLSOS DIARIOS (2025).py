...
#%%
corte_actual      = '20250131' #FUNCIONARÁ DESDE '20240229' EN ADELANTE

os.chdir('C:\\Users\\sanmiguel38\\Desktop\\DIANA LORENA\\montos desembolsados diarios')

tabla             = '[DESEMBOLSOS_DIARIOS].[dbo].[2025_01]'
tabla_acumulada   = '[DESEMBOLSOS_DIARIOS].[dbo].[2025_01_acum]'
...
fechas_2025 = pd.date_range(start = '2025-01-01', end = '2025-12-31')

# Crear un DataFrame con las fechas
df = pd.DataFrame({'Fecha': fechas_2025})
...
# Crear un DataFrame con las fechas
df_anterior = pd.DataFrame({'Fecha': fechas_2024})
...
# Convertir fechas de feriados a formato datetime
feriados_2024 = [datetime.strptime(fecha, '%d-%m-%Y') for fecha in feriados_2024]

# Función para etiquetar días no laborales
def dia_no_laboral(fecha):
    if fecha in feriados_2024:
        return 'no laboral'
...
if CARGA_SQL_SERVER == True:
    ...
    cursor.execute(f" SELECT top 100 * INTO {tabla} FROM DESEMBOLSOS_DIARIOS.DBO.[2024_01] ")    # se copia para tener la estructura
