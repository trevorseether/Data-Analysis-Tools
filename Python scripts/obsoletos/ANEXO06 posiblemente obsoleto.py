...
anexo_del_mes = "Rpt_DeudoresSBS Anexo06  - Abril2023 - campos ampliados.xlsx"
...
########################################################################################################################
archivo_refinanciados = 'REFINANCIADOS RECLASIFICADOS 30 04 2023.xlsx' #nombre del archivo de los refinanciados ####
########################################################################################################################
...
#%%
#modificar el alineamiento 15 para que se mantenga la calificación de los puchitos (menor a 100 o 1%)



#%%
# CÓDIGO VERIFICADOR DEL ALINEAMIENTO 15
...
#EL RESULTADO DEBE SER UN DATAFRAME VACÍO
...
df_resultado_2 = pd.read_excel('ANEXO Nº 6 Abril  2023.xlsx')
...
import os
import pandas as pd
os.chdir('C:\\Users\\sanmiguel38\\Desktop\\TRANSICION  ANEXO 6\\2023 abril\\anexo06 final') #cambiar ubicación

df_diferidos = pd.read_excel('Rpt_DeudoresSBS_Abril 2023 HT version 01.xlsx',
                 dtype={'Registro 1/': object, 
                        'Fecha de Nacimiento 3/': object,
                        'Código Socio 7/':object, 
                        'Número de Documento 10/': object,
...
df_diferidos = df_resultado_2.copy()

df_diferidos['Ingresos Diferidos 42/'].sum()
df_diferidos['Ingresos Diferidos 2'].sum()
...
Fincore'''] in ['00000681',
        ...
        return  df_diferidos['Provisiones Requeridas 36/'] * 0.642 # 0.50 es lo mínimo
...
df_diferidos['Intereses en Suspenso 41/'].sum()
...
print('variación de constituídas con el mes pasado', (suma_constituidas - float(7891147.64)).round(2))
...
#chequear, aún no está probado                                                                      
def calculo_52(df_diferidos):
    if df_diferidos['FEC_ULT_REPROG'] == '--':
        return df_diferidos['Saldo de colocaciones (créditos directos) 24/']
    else:
        return 0

df_diferidos['Saldo Capital de Créditos Reprogramados 52/'] = df_diferidos.apply(calculo_52, axis=1) #chequear, aún no está probado
#chequear, aún no está probado

#%%
#tipo de producto txt para hacer tablas dinámicas

def producto_txt(df_diferidos):
    if ((df_diferidos['Tipo de Producto 43/'] == 34) or \
        (df_diferidos['Tipo de Producto 43/'] == 35) or \
        (df_diferidos['Tipo de Producto 43/'] == 36) or \
        (df_diferidos['Tipo de Producto 43/'] == 38) or \
        (df_diferidos['Tipo de Producto 43/'] == 39)):
        return 'DXP'
    elif ((df_diferidos['Tipo de Producto 43/'] == 30) or \
        (df_diferidos['Tipo de Producto 43/'] == 31) or \
        (df_diferidos['Tipo de Producto 43/'] == 32) or \
        (df_diferidos['Tipo de Producto 43/'] == 33)):
        return 'LD'
    elif ((df_diferidos['Tipo de Producto 43/'] == 21) or \
        (df_diferidos['Tipo de Producto 43/'] == 22) or \
        (df_diferidos['Tipo de Producto 43/'] == 23) or \
        (df_diferidos['Tipo de Producto 43/'] == 24) or \
        (df_diferidos['Tipo de Producto 43/'] == 25) or \
        (df_diferidos['Tipo de Producto 43/'] == 29)):
        return 'MICRO'
    elif ((df_diferidos['Tipo de Producto 43/'] == 15) or \
        (df_diferidos['Tipo de Producto 43/'] == 16) or \
        (df_diferidos['Tipo de Producto 43/'] == 17) or \
        (df_diferidos['Tipo de Producto 43/'] == 18) or \
        (df_diferidos['Tipo de Producto 43/'] == 19)):
        return 'PEQUEÑA'
    elif ((df_diferidos['Tipo de Producto 43/'] == 95) or \
        (df_diferidos['Tipo de Producto 43/'] == 96) or \
        (df_diferidos['Tipo de Producto 43/'] == 97) or \
        (df_diferidos['Tipo de Producto 43/'] == 98) or \
        (df_diferidos['Tipo de Producto 43/'] == 99)):
        return 'MEDIANA'
    elif ((df_diferidos['Tipo de Producto 43/'] == 41) or \
        (df_diferidos['Tipo de Producto 43/'] == 45)):
        return 'HIPOTECARIA'    
    

df_diferidos['TIPO DE PRODUCTO TXT'] = df_diferidos.apply(producto_txt, axis=1) #chequear, aún no está probado



#%%
'CREACIÓN DEL EXCEL'
nombre = "ANX06 etiqueta de producto " + fecha_corte + ".xlsx"
try:
    ruta = nombre
    os.remove(ruta)
except FileNotFoundError:
