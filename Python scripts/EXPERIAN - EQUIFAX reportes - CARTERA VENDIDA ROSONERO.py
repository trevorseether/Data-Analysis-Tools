...
SELECT
	...
	ELSE RIGHT(CONCAT('0000000',p.numero),8)
		END                                           AS 'Numero de Crédito 18/',
        
	0                                                 AS 'Capital Vigente 26/' ,
	0                                                 AS 'Capital Refinanciado 28/',
	0                                                 AS 'Capital Vencido 29/',
	0                                                 AS 'Capital en Cobranza Judicial 30/',
	0                                                 AS 'Saldos de Créditos Castigados 38/',
    0                                                 AS 'REFINANCIADO_FILTRO'
FROM prestamo AS p
...
def arreglo_castigado(row):
    if row['MN Creditos Cartigados (*)'] > 0:
        ...
        return row['MN Deuda Directa Cobranza Judicial (*)']

# Aplica la función fila por fila
df_sentinel['MN Deuda Directa Cobranza Judicial (*)'] = df_sentinel.apply(arreglo_castigado, axis=1)
    
#%% nro fincore y nro fox
...
#merge
df_fincore = df_fincore.drop_duplicates(subset = 'cod pres para merge', keep = 'last')
...
if sin_match.shape[0] > 0:
    print(sin_match)
    print('investigar porque faltan créditos')
else:
    ''
...
#                                                   'Nro_Fincore'})
# ###############################################################################    
# anexo_06_descap = pd.DataFrame()
    
# anexo_06_descap['Nro_Fincore'] = 

# anexo_06_descap.columns
